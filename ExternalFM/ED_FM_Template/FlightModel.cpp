...
void FlightModel::L_stab()
{
	...
	*/
	...
	m_moment.x += m_q * ((Clb_b * m_corrBeta) + ((1.7 * Clda_b) * (((m_input.getRoll() * m_ailDeflection) + m_input.getTrimmAilR() - m_input.getTrimmAilL()) * m_ailDamage) + (m_lWingDamageCD + m_rWingDamageCD)) + ((0.55 * Cldr_b) * (m_input.getYaw() * m_rudDeflection)))
...
void FlightModel::N_stab()
{
	...
	*/
	...
	m_moment.y += m_q * (((3.1 * -Cnb_b) * m_corrBeta) + ((-Cnda_b * (m_input.getRoll() * m_ailDeflection)) * m_CnpStab) + ((1.2 * -Cndr_b) * (-m_input.getYaw() * m_rudDeflection)))
...
void FlightModel::thrustForce()
{
	...
	m_thrustForce = ((m_engine.updateThrust() + m_addThrust) * m_airframe.getEngineDamageMult() * m_thinAirMulti) * m_minusThrust;
...
void FlightModel::addedThrustCalc()
{
	...
	{
		m_addThrust = 0.5 * (m_engine.updateSpool() * (AdThrLAlt(m_state.m_mach) * AdThrLAltMulti(m_airframe.getAltInFeet())));
	}
	else
	{
		m_addThrust = 0.0;
...
void FlightModel::thrustTiltFunction()
{
	...
	{
		m_plusThrust = 0.55 * (m_engine.updateThrust() * m_airframe.getEngineDamageMult()); //0.376
	...
	{
		m_plusThrust = 1.66 * (m_engine.updateThrust() * m_airframe.getEngineDamageMult()); //0.888
	...
	{
		m_plusThrust =  1.77 * (m_engine.updateThrust() * m_airframe.getEngineDamageMult());
		m_minusThrust = 0.55;
	}
	else if ((m_airframe.getTiltEngineNozzlePosition() >= 0.8) && (m_airframe.getTiltEngineNozzlePosition() < 1.0))
	{
		m_plusThrust =  2.00 * (m_engine.updateThrust() * m_airframe.getEngineDamageMult());// war 1.88
		m_minusThrust = 0.05;//war 0.35
	}
	else if (m_airframe.getTiltEngineNozzlePosition() == 1.0)
	{
		m_plusThrust = 1.99 * (m_engine.updateThrust() * m_airframe.getEngineDamageMult());// war 2.05
...
{
	if ((m_airframe.getTiltEngineNozzlePosition() >= 0.4) && (m_state.m_mach <= 0.40) && (m_engine.getRPMNorm() >= 0.70) && (m_airframe.getWeightOnWheels() == 0.0))
	{
